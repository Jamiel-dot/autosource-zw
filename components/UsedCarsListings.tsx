...
interface UsedCarsListingsProps {
...
export const UsedCarsListings: React.FC<UsedCarsListingsProps> = ({ onCarSelect, initialFilters }) => {
  ...
  const fetchUsedCars = async (page: number) => {
    setLoading(true);
    try {
  ...
  const pageTitle = filters.make ? `${filters.make} Used Cars` : "Used Cars";
      ...
      <div className="flex flex-col lg:flex-row gap-10">
        ...
        <div className="flex-grow">
          <div className="mb-10">
            ...
            <p className="text-slate-500 text-sm font-medium uppercase tracking-widest">Quality pre-owned vehicles in Zimbabwe</p>
          </div>
          {loading ? (
            ...
            <div className="py-24 text-center bg-white rounded-[40px] border-2 border-dashed border-slate-100">
              <p className="text-slate-500 font-bold italic">No used vehicles matching your criteria.</p>
            </div>
          )}
        </div>
      </div>
